...
export function buttonSubmit() {
  form.addEventListener("submit", (e) => {
    ...
    document
      .querySelectorAll(".input__icon--message")
      .forEach((el) => el.remove());
    document
      .querySelectorAll(".border--error")
      .forEach((input) => input.classList.remove("border--error"));
    document
      .querySelectorAll(".email--error")
      .forEach((input) => input.classList.remove("email--error"));

    const nameValid = firstName(true);
    const lastNameValid = lastName(true);
    const emailValid = email(true);
    const passValid = password(true);
    ...
    if (!isValid) {
      ...
      const fields = [
        ...
        {
          valid: lastNameValid,
          element: document.querySelector(".last__name"),
        },
